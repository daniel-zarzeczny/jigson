...
public class JsonObjectFilter implements JsonFilter<JsonObject, JsonElement> {
    ...
    @Override
    public JsonElement filter(final JsonObject jsonObject, final String criterion, final Context context) {
        final Boolean isAccepted = JsonExpression.from(jsonObject, criterion).withContext(context).interpret();
